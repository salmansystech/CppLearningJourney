...
// Checks if all the letters are included in the list of guessed letters
...
void clean_screen() {
    ...
    for (int i = 0; i < 100; ++i) {
        cout << endl;  // Corrected the output operator
    }
}

bool is_word_already_guessed(string secret, string guessed) {  // Fixed function name
    // Going through all the characters in the secret string.
    for (string::size_type index = 0; index < secret.size(); ++index) {  // Changed <= to < for correct bounds
        ...
        if (guessed.find(secret.at(index)) == string::npos) {  // Fixed comparison operator
            return false;
        }
    }
...
void print_game_status(string secret, string guessed) {
    ...
    // This is an alternative for the for loop with an index variable
