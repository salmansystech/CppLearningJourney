...
// Writes the data to the file, ordering by user IDs.
bool write_data(const string& file_name,
                const map<string, shared_ptr<Student>>& alphabetical_order) {
    ...
    for (const auto& pair : alphabetical_order) {
        shared_ptr<Student> student = pair.second;
    ...
    while( true ) {
        ...
        // Changing student data
            ...
            string student_number = parts[1];
                        ...
                        shared_ptr<Student> student = student_numbers[student_number];
                        ...
                        if (is_valid_phone_number(new_phone_number)) {
                            ...
                            if (not write_data(file_name, user_ids)) {
